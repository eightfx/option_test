...
use super::structs::{OptionTick, OptionChain, OptionBoard, StrikeBoard, FloatType, OptionType, OptionValue, OptionSide, OptionBase};
...
impl ExtractCommonInfo for OptionTick{
	fn strike(&self) -> Result<FloatType>{Ok(self.strike)}
	fn maturity(&self) -> Result<DateTime<Utc>>{Ok(self.maturity)}
	fn asset_price(&self) -> Result<FloatType>{Ok(self.asset_price)}
	fn risk_free_rate(&self) -> Result<FloatType>{Ok(self.risk_free_rate)}
	fn dividend_yield(&self) -> Result<FloatType>{Ok(self.dividend_yield)}
	fn option_type(&self) -> Result<OptionType>{Ok(self.option_type.clone())}
	fn option_value(&self) -> Result<OptionValue>{Ok(self.option_value.clone())}
}

impl ExtractCommonInfo for StrikeBoard{
	fn strike(&self) -> Result<FloatType>{Ok(self.0[0].strike().unwrap())}
	fn maturity(&self) -> Result<DateTime<Utc>>{Ok(self.0[0].maturity().unwrap())}
	fn asset_price(&self) -> Result<FloatType>{Ok(self.0[0].asset_price().unwrap())}
	fn risk_free_rate(&self) -> Result<FloatType>{Ok(self.0[0].risk_free_rate().unwrap())}
	fn dividend_yield(&self) -> Result<FloatType>{Ok(self.0[0].dividend_yield().unwrap())}
	fn option_type(&self) -> Result<OptionType>{Ok(self.0[0].option_type().unwrap())}
